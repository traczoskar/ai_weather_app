...
const Container = ({ children, isCollapsed, isButton }: ContainerProps) => {
  return (
    <>
      ...
      {isButton && (
        <div
          className={`sm:px-6 sm:py-3 px-7 py-2 flex bg-white hover:scale-[1.01] hover:bg-sky-50 dark:hover:bg-sky-700 dark:hover:brightness-105 border dark:bg-sky-800 dark:border-x-0 dark:border-b-0 border-sky-50 dark:border-t dark:border-t-sky-700 transition-all sm:p-14 p-7 rounded-2xl w-full shadow-md  dark:shadow-lg`}
        >
          {children}
        </div>
      )}
